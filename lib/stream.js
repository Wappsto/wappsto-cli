...
module.exports = class Stream {
  constructor(wappsto, installation = undefined, remote = true) {
  ...
  async getAll(session) {
    let result = false;
    try {
      const response = await HTTP.get(`${Config.host()}/services/2.0/stream?expand=2`, {
        headers: {
  ...
  async create(subscription, session) {
    let result = {};
    try {
      const response = await HTTP.post(`${Config.host()}/services/2.0/stream`, {
        subscription,
      }, {
        headers: {
          'x-session': session || this.wappsto.session.get(),
  ...
  printConsoleMessage(data, callback) {
    if (!this.remote) {
      return;
    }
  ...
  async handleNotification(data, callback) {
    if (data.read !== 'unread') {
      return;
    }
    ...
    try {
      ...
      case 'console':
        if (event.type === 'error') {
          ...
          callback({ error: msg, type: 'Background', timestamp: event.timestamp });
          break;
        }
        // falls through
      default:
