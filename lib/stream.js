...
export default class Stream {
  ...
  constructor(wappsto, callback, subs, session) {
    ...
    this.subs = subs;
    this.host = Config.host().split('//').pop();
    this.callback = callback;
    this.session = session;
    this.url = `wss://${this.host}/services/2.1/websocket/open`;
    this.sock = undefined;
  }
  ...
  open() {
    const connect = () => {
      this.sock = new WebSocket(`${this.url}?x-session=${this.session || this.wappsto.session.get()}`);

      if (this.sock.on === undefined) {
        this.sock.on = (name, fun) => {
          this.sock[`on${name}`] = fun;
        };
      }

      this.sock.on('close', (pCode, pMsg) => {
        if (this.closed) {
          return;
        }
        let code = pCode;
        ...
        switch (code) {
          case 1005:
            break;
          default:
            tui.showError(`Stream closed: URL: ${this.url} Reason: ${msg} Code: ${code}`);
      ...
      this.sock.on('message', (data, isBinary) => {
        let message = isBinary !== false ? data : data.toString();
