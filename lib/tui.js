...
module.exports = {
    ...
    showError: /* istanbul ignore next */ (msg, err) => {
        process.stderr.write(`${chalk.red('!')} ${chalk.red.bold(msg)}\n`);
        if (err) {
            if (err.response && err.response.data) {
                process.stderr.write(`data: ${JSON.stringify(err.response.data)}\n`);
            } else if (typeof err === 'string') {
                process.stderr.write(`string: ${err}\n`);
            } else {
                process.stderr.write(`def: ${JSON.stringify(err)}\n`);
            }
        }
    },

    write: /* istanbul ignore next */ (msg) => {
