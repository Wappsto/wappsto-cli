...
module.exports = class Wappsto {
    constructor() {
        ...
        if (process.env.WAPPSTO_HOST) {
            const eHost = `https://${process.env.WAPPSTO_HOST}`;
            this.conf.set('host', eHost);
        }
        this.HOST = this.conf.get('host');
        if (!this.HOST) {
            this.HOST = 'https://wappsto.com';
        }
    }
    ...
    async getVersion(id) {
        try {
            const response = await axios.get(`${this.HOST}/services/version/${id}?verbose=true`);
            return response.data;
        } catch (err) {
            console.error(err);
            return {};
        ...
        try {
            ...
            data.forEach((event) => {
                ...
                case 'state':
                    break;
                case 'installation':
                    callback({ status: 'Installation Updated', session: true });
                    break;
                case 'extsync':
                    try {
    ...
    async find(type, search, method, quantity, not_shared) {
        try {
            const url = `${type}?expand=0&${search}&method=[${method}]&quantity=${quantity}&not_shared_with=${not_shared}`;
            const response = await axios.get(`${this.HOST}/services/${url}`);
            return response.data;
        } catch (err) {
            console.error('Failed to find', err);
            console.log(err.response.data.data.validation_error);
        }
    }
};
