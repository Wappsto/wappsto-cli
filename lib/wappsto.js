...
module.exports = class Wappsto {
    ...
    openStream(env, id, callback, session) {
        ...
        const ws = new WebSocket(`wss://${host}/services/stream/${id}?x-session=${ses}`,
            {
                origin: `https://${host}`,
            });
            ...
            try {
                ...
                data.forEach((event) => {
                    ...
                    case 'state':
                        break;
                    case 'installation':
                        callback('Installation Updated');
                        break;
                    case 'extsync':
                        try {
                            const msg = JSON.parse(ev.body);
                            if (msg.key === 'log') {
                                callback(`Background - ${msg.arguments[0]}`);
                            }
                        } catch (err) {
                            callback(ev.body);
                        }
