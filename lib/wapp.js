...
module.exports = class Wapp {
    ...
    async create() {
        ...
        if (data) {
            ...
            await this.wappsto.createInstallation(data.version[0].meta.id);
        }
        status.stop();
    ...
    static compareVersions(oldVersion, newVersion) {
        ...
        for (let i=0; i<keys.length; i++) {
            let key = keys[i];
            let obj1 = oldVersion[key];
            let obj2 = newVersion[key];
            if(typeof oldVersion[key] === 'string') {
                if(obj1 != obj2) {
                    return true;
                }
            } else if(Array.isArray(oldVersion[key])) {
                if (obj1.length == obj2.length && obj1.every(function(u, i) {return u === obj2[i];})) {
                    return true;
                }
            } else {
                if (Wapp.compareVersions(oldVersion[key], newVersion[key])) {
                    return true;
                }
            }
        }
    ...
    async update(reinstall) {
        ...
        if (remoteVersion.meta.revision !== this.application.version[0].revision && !Wapp.compareVersions(this.manifest, remoteVersion)) {
    ...
    async handleStreamEvent(data) {
        ...
        if (data.reinstall) {
            ...
            if (items.length) {
                ...
                tui.showWarning("Failed to find anything matching the permission request");
            }
            unblock();
