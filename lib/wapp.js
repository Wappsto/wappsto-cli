...
module.exports = class Wapp {
    constructor() {
        ...
        this.setData(files.loadJsonFile(`${this.cacheFolder}/application`));
    ...
    async create() {
        ...
        case 'generate':
            ...
            this.setData(await this.wappsto.createApplication(this.manifest));
            if (!this.application) {
                status.stop();
        ...
        default:
            ...
            if (data) {
                ...
                await this.installation.create(data.version[0].meta.id);
    ...
    async update(reinstall) {
        ...
        const remoteVersion = await this.version.get();
        if (remoteVersion.meta.revision !== localVersion.revision
            && !Util.compareVersions(this.manifest, remoteVersion)) {
            status.stop();
        ...
        if (upload) {
            await this.version.update(this.manifest);
        }


        // Find all files on disk
        this.wapp_folders.forEach((f) => {
            localFiles = localFiles.concat(files.getAllFiles(f, Util.validateFile));
        });
        ...
        for (let i = 0; i < allFiles.length; i += 1) {
            ...
            const rf = remoteVersion.file.find(file => filePath === `${Util.getFilePath(file.use)}/${file.name}`);
            const lf = localVersion.file.find(file => filePath === `${Util.getFilePath(file.use)}/${file.name}`);
        ...
        if (reinstall) {
            results.push(await this.installation.restart(true, true));
        } else {
            results.push(await this.installation.restart());
        }
        ...
        if (answer.del) {
            ...
            if (answer.remote) {
                ...
                this.application.version.forEach((v) => {
                    if (v.meta && v.meta.id) {
                        results.push(this.version.delete());
                        results.push(this.installation.delete(v.meta.id));
                    }
                });
