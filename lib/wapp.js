...
module.exports = class Wapp {
    ...
    async create() {
        ...
        if (data) {
            ...
            this.wappsto.createInstallation(data.meta.id);
    ...
    async openStream() {
        ...
        for (let i = 0; i < streams.length; i += 1) {
            if (!light_stream && streams[i].subscription.indexOf('/extsync') !== -1) {
                light_stream = streams[i].meta.id;
                break;
            }
        }
        ...
        if (!app_stream) {
            ...
            app_stream = newStream.meta.id;
        }

        if (!light_stream) {
            const newStream = await this.wappsto.createStream(['/extsync']);
            light_stream = newStream.meta.id;
        }

        this.wappsto.openStream('app.', app_stream, callback);
        this.wappsto.openStream('light.', light_stream, callback, this.installation_session);
    }
