...
module.exports = class Wapp {
    ...
    async update(reinstall) {
        ...
        for(let i=0; i<allFiles.length; i+=1) {
            ...
            const rf = remoteVersion.file.find(file => path === `${file.use}/${file.name}`);
            const lf = localVersion.file.find(file => path === `${file.use}/${file.name}`);
            ...
            //console.debug(`${path} LF ${!!lf} RF ${!!rf} LU ${locallyUpdated} RU ${remoteUpdated} F ${!!fileTime}`);
