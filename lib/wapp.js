...
module.exports = class Wapp {
    ...
    async create() {
        ...
        case 'generate':
            ...
            this.application = new Application(await this.wappsto.createApplication(this.manifest));
            if (!this.application) {
                status.stop();
            ...
            this.manifest = Wapp.saveApplication(this.application.data);
            status.stop();
    ...
    async uploadFile(filePath) {
        ...
        const localFile = localVersion.file.find((file) => filePath === `${Util.getFilePath(file.use)}/${file.name}`);

        await this.wappsto.updateFile(this.versionID, localFile.meta.id, filePath);
    ...
    async update(reinstall) {
        ...
        for (let i = 0; i < allFiles.length; i += 1) {
            ...
            const rf = !remoteVersion ? null : remoteVersion.file.find((file) => filePath === `${Util.getFilePath(file.use)}/${file.name}`);
        ...
        await new Promise((resolve) => {
            setTimeout(async () => {
                const newApp = await this.wappsto.loadApplication(this.application.id);
        ...
        if (answer.del) {
            ...
            if (answer.remote) {
                ...
                if (this.application.id) {
                    results.push(this.wappsto.deleteApplication(this.application.id));
                }
                await Promise.all(results);
            }
