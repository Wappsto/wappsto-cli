...
module.exports = class Wapp {
    ...
    get versionID() {
        return this.application.version[0].meta.id;
    }
    ...
    async create() {
        ...
        case 'generate':
            ...
            this.application = await this.wappsto.createApplication(this.manifest);
            if (!this.application) {
                status.stop();
                throw new Error('Failed to generate Application');
            }

            installation = await this.wappsto.createInstallation(this.versionID);
            ...
            if (this.application && installation) {
                tui.showMessage(`Wapp created with id: ${this.application.meta.id}`);
            }
            break;
        default:
            this.deleteLocal();
            ...
            data = await this.wappsto.createApplication(newWapp);
            if (data) {
                this.createFolders(newWapp.features, newWapp.examples);
                installation = await this.wappsto.createInstallation(data.version[0].meta.id);
                Wapp.saveInstallation(installation);
    ...
    createFolders(folders, createExp) {
        ...
        const modulePath = path.dirname(require.resolve('wappsto-cli'));

        dirs.concat(this.wapp_folders_create).forEach((f) => {
            files.createFolders(`${f}/.`);
            if (createExp) {
                if (exFiles[f]) {
                    const exPath = `${modulePath}/../examples/${f}`;
    ...
    async downloadWapp(app) {
        ...
        for (let i = 0; i < app.version[0].file.length; i += 1) {
            ...
            const filePath = `${Wapp.getFilePath(file.use)}/${file.name}`;
            try {
    ...
    async update(reinstall) {
        ...
        const remoteVersion = await this.wappsto.getVersion(this.versionID);
        ...
        for (let i = 0; i < allFiles.length; i += 1) {
            ...
            let fileTime = null;
            const filePath = `${Wapp.getFilePath(f.use)}/${f.name}`;

            const rf = remoteVersion.file.find(file => filePath === `${Wapp.getFilePath(file.use)}/${file.name}`);
            const lf = localVersion.file.find(file => filePath === `${Wapp.getFilePath(file.use)}/${file.name}`);
    ...
    async getInstallation(name) {
        ...
        if (name) {
            ...
            installation = await this.wappsto.getInstallation(this.versionID);
        }
        return installation;
    }
