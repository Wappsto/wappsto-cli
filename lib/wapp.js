...
class Wapp {
  constructor(verbose = false, remote = true) {
    tui.verbose = verbose;
  ...
  initCacheFolder() {
    if (!files.directoryExists(this.cacheFolder)) {
      files.createFolders(this.cacheFolder);
    }
    ['application', 'installation', 'session'].forEach((e) => {
      const f = `.${e}`;
      if (files.fileExists(f)) {
        files.moveFile(f, `${this.cacheFolder}${e}`);
      }
    });
  }
  ...
  async create(validate) {
    ...
    default:
      ...
      if (this.application) {
        ...
        status.stop();
        await this.createFolders(newWapp.features, newWapp.examples, customFolders);
        status.start();
    ...
    if (files.fileExists('.gitignore')) {
      let ignore = files.loadFile('.gitignore');
      let addLines = '';
      ...
      if (addLines !== '') {
        ignore += `\n${addLines}`;
        files.saveFile('.gitignore', ignore);
      }
    } else {
      files.saveFile('.gitignore', this.ignore_file);
    }
  }

  async createFolders(folders, createExamples, folderMapping = {}) {
      ...
      if (createExamples && exampleFiles[f]) {
        ...
        if (overwrite === undefined) {
          for (let j = 0; j < exampleFiles[f].length; j += 1) {
            if (files.fileExists(`${path}/${exampleFiles[f][j]}`)) {
              /* eslint-disable-next-line no-await-in-loop */
              const answer = await tui.askOverwriteFiles();
        ...
        for (let j = 0; j < exampleFiles[f].length; j += 1) {
          ...
          if (!files.fileExists(`${path}/${file}`) || overwrite === true) {
  ...
  async configure() {
    ...
    const answer = await tui.configureWapp(app.oauth_external, app.oauth_client);
