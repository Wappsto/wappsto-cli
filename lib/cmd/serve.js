...
import { directoryExists, fileExists } from '../files.js';
...
function webFileExists(dir, request) {
...
async function startForegroundServer(optPort, sessionID, tokenID, nobrowser) {
  const port = optPort || Config.port();
  const newPort = await detect(port);

  if (port !== newPort) {
    tui.showWarning(`Port ${port} is in use, switching to ${newPort}`);
  }
    ...
    middleware: [
      function localServe(request, response, next) {
        ...
        try {
          ...
          if (webFileExists(Config.foreground(), request)) {
            next();
          } else {
...
async function startRemoteBackgroundRunner(wapp) {
