...
class OpenWeatherMap {
  ...
  async getCurrentData(lat, lon) {
    let res = await axios.get(
      `${this.url}/data/2.5/weather${this.query}&lat=${lat}&lon=${lon}`
    );
    return res.data;
  }
  ...
  async getForecast(lat, lon, count) {
    let res = await axios.get(
      `${this.url}/data/2.5/forecast${this.query}&lat=${lat}&lon=${lon}&cnt=${count}`
    );
    return res.data;
  }
  ...
  async getCity(city) {
    let res = await axios.get(
      `${this.url}/geo/1.0/direct?q=${city}&limit=1&appid=${this.key}`
    );
    return res.data;
  }
