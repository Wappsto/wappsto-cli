...
async function handleWeatherData(event) {
    ...
    if(event.current) {
        ...
        document.getElementById("weather_icon").src = "http://openweathermap.org/img/wn/" + weather.icon + "@2x.png";
    }
    ...
    if(event.network) {
        ...
        if(city_value) {
            ...
            document.getElementById("city_article").attributes["aria-busy"].value = false;
